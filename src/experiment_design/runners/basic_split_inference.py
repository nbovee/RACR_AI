"""
The two runners defined here implement basic split inference behavior between a single "client"
...
from experiment_design.tasks import tasks
from runner import BaseRunner


class ClientRunner(BaseRunner):
    """
    To define the way our client behaves, there are only three parts of the BaseRunner class we
    have to overwrite:
        1.) The `partners` class attribute should list the names of the nodes we want to handshake
            with before the runner starts
    ...
    When the experiment runs, this runner will actually respond to an instance of 
    ...
    def inference_sequence_per_input(self, task: tasks.SingleInputInferenceTask):
        ...
        while current_split_layer < splittable_layer_count:
            ...
            self.node.get_connection(self.DOWNSTREAM_PARTNER).give_task(downstream_task)

    def on_finish(self):
        downstream_finish_signal = tasks.FinishSignalTask(self.node.node_name)
        self.node.get_connection(self.DOWNSTREAM_PARTNER).give_task(downstream_finish_signal)


class EdgeRunner(BaseRunner):
    """
    Defining our edge node's behavior is much simpler because it only has to respond to instances
    of the `SimpleInferenceTask` class, which is already defined in the BaseRunner class. The 
